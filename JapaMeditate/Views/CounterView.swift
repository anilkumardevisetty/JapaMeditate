...
private extension CounterView {
    ...
    var counterCircle: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                Circle()
                    .stroke(Color.white.opacity(0.25), style: StrokeStyle(lineWidth: 14))
    ...
    func beadsRing(size: CGFloat) -> some View {
        ZStack {
            ForEach(0..<108, id: \.self) { i in
                let point = positionForBead(in: size * 1.05, index: i)
                Circle()
                    .fill(beadStates[i] ? Color.white : Color.white.opacity(0.25))
                    .frame(width: beadStates[i] ? 10 : 8, height: beadStates[i] ? 10 : 8)
                    .shadow(color: beadStates[i] ? Color.white.opacity(0.4) : .clear,
    ...
    var mantraPreviewTile: some View {
...
private extension CounterView {
    ...
    func syncBeadsWithCurrentCount() {
        ...
        if capped > 0 {
            for i in 0..<capped { newStates[i] = true }
        }
